...
public class Perlin_noise extends PApplet {
    ...
    String audioFileName = "[YT2mp3.info] - Assassin_'s Creed II   Ezio_'s Family (Dubstep Remix) Remake (320kbps)"; // Audio
                                                                                                                     // file
                                                                                                                     // in
                                                                                                                     // data
                                                                                                                     // folder
    ...
    float fps = 30;
    float smoothingFactor = (float) 0.25; // FFT audio analysis smoothing factor
    // ------------------------
    ...
    void drawAll(float[] sum) {
        ...
        while (x < width * 1.5 && x > 0 - width / 2) {
            ...
            float addon = (float) (frameCount * 1.5);
        ...
        float extendingLinesMin = (float) (sphereRadius * 1.3);
        float extendingLinesMax = (float) (sphereRadius * 3.5);
        ...
        for (int angle = 0; angle <= 240; angle++) {
            ...
            float extendingSphereLinesRadius = map(noise((float) (angle * 0.3)), 0, 1, extendingLinesMin, extendingLinesMax);
            ...
            if (sum[0] != 0) {
                if (angle >= 0 && angle <= 30) {
                    extendingSphereLinesRadius = map(sum[240 - round(map((angle), 0, 30, 0, 80))], 0, (float) 0.8,
                    ...
                    extendingSphereLinesRadius = map(sum[160 + round(map((angle - 210), 0, 30, 0, 80))], 0, (float) 0.8,
                            extendingSphereLinesRadius - extendingSphereLinesRadius / 8, extendingLinesMax * 1.5f); // Highs
                }
            }
        ...
        for (int groundX = 0; groundX <= width; groundX++) {
            ...
            noStroke();
            fill(255);
            circle(groundX, groundY, (float) (1.8 * unit / 10.24));
            noFill();
        }
    }
